...
export const createMockProto = (options = {}) => {
  const { syntax = "proto3", packageName = "", messages = [], enums = [], services = [] } = options;
...
expect.extend({
  toContainPlantUMLStructure(received) {
    ...
    if (pass) {
      ...
      return {
        message: () =>
          `expected ${received} to contain valid PlantUML structure (missing: ${!hasStart ? "@startuml" : !hasEnd ? "@enduml" : "!pragma"})`,
        pass: false,
      };
    }
  },
  ...
  toHaveUniqueRelationships(received) {
    ...
    const relationshipLines = lines.filter(
      (line) => line.includes("--") && (line.includes("--*") || line.includes("--o") || line.includes("-->"))
    );
    ...
    if (pass) {
      ...
      const duplicates = relationshipLines.filter((line, index) => relationshipLines.indexOf(line) !== index);
      return {
  ...
  toHaveOptionalMarkers(received, fieldNames) {
    const pass = fieldNames.every((fieldName) => received.includes(`${fieldName} : `) && received.includes("?"));
    ...
    if (pass) {
      ...
      const missing = fieldNames.filter(
        (fieldName) => !received.includes(`${fieldName} : `) || !received.includes("?")
      );
      return {
